...
BRAND_ACCENT = "#c8102e"  # czerwień ACB
...
def fetch_sofa_games_today(season_id, today_slug):
    """Pobiera mecze ACB z Sofascore na dziś."""
    games = []
    ...
    if USE_AI_PREDICTIONS:
        ...
        if result:
            ...
            _ai_log.append({
                ...
                "phase": ((ev.get("roundInfo") or {}).get("name") or "?"),
                "date": datetime.fromtimestamp(ts, CET).isoformat() if ts else "",
...
def save_picks(picks, today_slug):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("\n".join(picks))
    print(f"   Zapisano {len(picks)} typów do {path}")
...
def main():
    ...
    if not season_id:
        print("!! Brak season_id ACB - pusta strona")
